...
static void connect_run(struct etpan_thread_op * op)
{
	...
	r = do_mailimap_socket_connect(param->imap,
				    param->server, param->port, param->socks_info);
	
	result->error = r;
}


int imap_threaded_connect(Folder * folder, const char * server, int port, SocksInfo *socks_info)
{
	struct connect_param param;
	struct connect_result result;
	chashdatum key;
...
int imap_threaded_connect_ssl(Folder * folder, const char * server, int port, SocksInfo *socks_info)
{
	struct connect_param param;
	struct connect_result result;
	chashdatum key;
